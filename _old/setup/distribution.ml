(** @canonical Game.Setup.Distribution *)
...
let available_roles (x : pull_role) : Roles.Role.t list =
  Roles.Map.fold
    (fun k v acc ->
      if Roles.Role.Kind.equal x.role (Roles.Role.kind k) && Bool.not v
      then k :: acc
      else acc)
    x.map
    []
...
let show_available_roles (x : Roles.Role.Kind.t) (xs : Roles.Role.t list)
  : string
  =
  Printf.sprintf
    "available %s:%s\n"
